<template>
  <nav class="w-full px-4 py-2 sm:px-5 md:px-10 xl:px-20 2xl:px-40 sm:py-3 mb-1 bg-gray-100">
    <div class="flex items-center justify-between">
      ...
      >
        <div @keydown.esc="menuIsOpen = false" v-show="menuIsOpen" class="z-10 fixed inset-0 transition-opacity">
            <div @click="menuIsOpen = false" class="absolute inset-0 bg-black opacity-50" tabindex="0"></div>
        </div>
      </transition>
      <!-- Drawer Menu -->
      <aside class="p-5 transform top-0 left-0 w-80 bg-white fixed h-full overflow-auto ease-in-out transition-all duration-300 z-30" :class="menuIsOpen ? 'translate-x-0' : '-translate-x-full'"> 
        <div class="close">
          <button aria-label="close menu" class="absolute top-0 right-0 mt-4 mr-4" @click=" menuIsOpen = false">
            <svg 
              class="w-6 h-6"
              fill="none" stroke-linecap="round" 
              stroke-linejoin="round" stroke-width="2"
        ...
        <div class="follow pt-6">
          ...
          <div class="social flex space-x-10 mt-2">
            ...
            <a href="https://www.facebook.com/UniversitedeLille/" target="_blank" rel="noopener noreferrer" title="Lien Facebook université lille">
              <svg 
                title="Lien Facebook université lille"
                aria-hidden="true" focusable="false" 
            ...
            <a href="https://www.instagram.com/univ_lille/" target="_blank" rel="noopener noreferrer" title="Lien Instagram université lille">
              <svg 
                title="Lien Instagram université lille"
                aria-hidden="true" focusable="false" 
            ...
            <a href="https://www.youtube.com/channel/UCwQRR1SjMMfwdt0NgvFCTAg" target="_blank" rel="noopener noreferrer" title="Lien Youtube université lille">
              <svg 
                title="Lien Youtube université lille"
                aria-hidden="true" focusable="false" 
...
export default {
  ...
  watch: {
    menuIsOpen: {
      ...
      handler(menuIsOpen) {
        if (process.client) {
          if (menuIsOpen) document.body.style.setProperty("overflow", "hidden");
  ...
  mounted() {
    ...
    document.addEventListener("keydown", e => {
      if (e.keyCode === 27 && this.menuIsOpen) this.menuIsOpen = false;
    });
  }
};
</script>
